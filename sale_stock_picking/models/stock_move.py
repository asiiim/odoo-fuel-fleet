        ...
        for vals in vals_list:
            if not "picking_id" in vals:
                ...
                if "location_id" in vals and vals["location_id"]:
                    if (
                        picking_type_rec.default_location_src_id.id
                        != vals["location_id"]
                    ):
                        vals.update(
                            {"location_id": picking_type_rec.default_location_src_id.id}
                        )
                if "location_dest_id" in vals and vals["location_dest_id"]:
                    if (
                        picking_type_rec.default_location_dest_id.id
                        != vals["location_dest_id"]
                    ):
                        vals.update(
                            {
                                "location_dest_id": picking_type_rec.default_location_dest_id.id
                            }
                        )
